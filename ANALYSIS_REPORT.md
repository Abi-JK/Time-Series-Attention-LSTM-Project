...
* **Multivariate Dependence:** The `target` variable is calculated as a **weighted sum** of the auxiliary features, ensuring that the features are non-trivially related to the prediction task, which justifies the use of a multivariate (Multi-Input Multi-Output) model like the LSTM.
...
* **Custom Layer:** The `AdditiveAttentionLayer` (Requirement 2) takes $H_t$ and $S_t$ as inputs. It calculates the **alignment score** using two weight matrices ($W$ and $U$) and a score vector ($V$), applies **Softmax** across the time dimension to get attention weights ($\alpha_t$), and computes the **Context Vector** ($C_t$) as the weighted sum of $H_t$.
...
***[**Note: Replace the bracketed placeholders above with the actual results printed by the script.**]***
...
2.  **Selective Focus:** The attention mechanism's ability to selectively weigh the most relevant past time steps for the current forecast (as demonstrated in Section 4) proves crucial in modeling the non-linear, long-range dependencies inherent in the synthetic data.
...
The attention weights were analyzed across the entire test set to understand the model's temporal focus (Requirement 4).
...
### Visualizations (Requirement 3 & 4)
